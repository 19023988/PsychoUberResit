                     ...
                     if len(self._taxis) > taxiIdx:
                         ...
                         if bidderNode is not None:
                             ...
                             NoTaxiFares = len([fare for fare in self._taxis[allocatedTaxi]._availableFares.values() if
                                                fare.allocated])
                             NoIndexFares = len(
                                 [fare for fare in self._taxis[taxiIdx]._availableFares.values() if fare.allocated])
                     ...
                     if len(self._taxis) > taxiIdx:
                         ...
                         if bidderNode is not None:
                             ...
                             if (AccountMax - AccountMin) == 0:
                                 ...
                                 taxiAccountSeverity = 1 - (
                                             (self._taxis[taxiIdx]._account - AccountMin) / (AccountMax - AccountMin))
                             ...
                             if (DistanceMax - DistanceMin) == 0:
                                 ...
                                 taxiDistanceSeverity = 1 - (
                                             (self._parent.distance2Node(bidderNode, fareNode) - DistanceMin) / (
                                                 DistanceMax - DistanceMin))

                             taxiPassengerSeverity = 1 - int((self._taxis[taxiIdx]._passenger == None))

                             taxiSeverity = (taxiAccountSeverity * AccountWeight) + (taxiFareSeverity * FareWeight) + (
