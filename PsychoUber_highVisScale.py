...
# colours for the overall draw. These can be set to make things more visible as desired.
backgroundColour = pygame.Color(0,0,0) # default to black background
lineColour = pygame.Color(255,192,0) # default lines in yellow-orange
junctionColour = pygame.Color(192,144,0) # default junctions in darker yellow-orange
fareColour = pygame.Color(255,64,128) # default fares in pink
lineWidth = 5 # control width of drawn lines in pixels
taxiScale = 1 # scale size of taxi symbols (circles)
fareScale = 2 # scale size of fare symbols (triangles)
...
displayedBackground.fill(backgroundColour)
...
meshSize = ((activeSize[0]//worldX),(activeSize[1]//worldY))
...
for jct in range(len(junctionIdxs)):
    jctSquares[jct].fill(junctionColour)
    ...
    pygame.draw.rect(jctSquares[jct],lineColour,pygame.Rect(0,0,round(meshSize[0]/2),round(meshSize[1]/2)),5)
...
taxiColours = {}
# possible colours for taxis: white, blue, green, red, magenta, cyan, yellow, black
taxiPalette = [pygame.Color(255,255,255),
...
taxiRect = pygame.Rect(round(meshSize[0]/3)*taxiScale,
                       round(meshSize[1]/3)*taxiScale,
                       round(meshSize[0]/3)*taxiScale,
                       round(meshSize[1]/3)*taxiScale)

fareRect = pygame.Rect(round(3*meshSize[0]/8)*fareScale,
                       round(3*meshSize[1]/8)*fareScale,
                       round(meshSize[0]/4)*fareScale,
                       round(meshSize[1]/4)*fareScale)
          ...
          if 'time' in outputValues and len(outputValues['time']) > 0 and curTime != outputValues['time'][-1]:
             ...
             if len(faresToRedraw) > 0:
                for fare in faresToRedraw.items():
                    ...
                    # fares are plotted as pink triangles (using pygame's points representation which
                    # is relative to the rectangular surface on which you are drawing)
                    pygame.draw.polygon(drawPositions[fare[0][0]][fare[0][1]],
                                        fareColour,
                                        [(meshSize[0]/2,meshSize[1]/2-2*math.sin(math.pi/6)*meshSize[1]/4*fareScale),
                                         (meshSize[0]/2-math.cos(math.pi/6)*meshSize[1]/4*fareScale,meshSize[1]/2+math.sin(math.pi/6)*meshSize[1]/4*fareScale),
                                         (meshSize[0]/2+math.cos(math.pi/6)*meshSize[1]/4*fareScale,meshSize[1]/2+math.sin(math.pi/6)*meshSize[1]/4*fareScale)])
